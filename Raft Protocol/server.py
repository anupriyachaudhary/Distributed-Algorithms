...
#                print(list(self.transactionPool.queue))
    ...
    def isTxnsAppendable(self, txns, uuids, clients, addresses):
        ...
        if isTxn1AlreadyInLog == True and isTxn2AlreadyInLog == False:
            lock.acquire()
            if uuids[1] not in self.transactionIDs:
                self.transactionIDs.append(uuids[1])
            txns = txns[1].split('-')
            txns = '*'.join(txns)
            txnData = uuids[1] + '+' + addresses[1][0] +  '?' + str(addresses[1][1]) + '+' + txns
            self.transactionPool.put(txnData)
            lock.release()
        
        if isTxn2AlreadyInLog == True and isTxn1AlreadyInLog == False:
            lock.acquire()
            if uuids[0] not in self.transactionIDs:
                self.transactionIDs.append(uuids[0])
            txns = txns[0].split('-')
            txns = '*'.join(txns)
            txnData = uuids[0] + '+' + addresses[0][0] +  '?' + str(addresses[0][1]) + '+' + txns
            self.transactionPool.put(txnData)
            lock.release()
        ...
        if isTxn1Conflict == True and isTxn2Conflict == False:
            lock.acquire()
            if uuids[1] not in self.transactionIDs:
                self.transactionIDs.append(uuids[1])
            txns = txns[1].split('-')
        ...
        if isTxn2Conflict == True and isTxn1Conflict == False:
            lock.acquire()
            if uuids[0] not in self.transactionIDs:
                self.transactionIDs.append(uuids[0])
            txns = txns[0].split('-')
    ...
    def isTxnAlreadyInLog(self, uuids, clients, addresses):
    ...
    def printBlockChain(self):
        ...
        for index in range(0, len(self.log)):
            ...
            print(80*h)
            if index<10:
                print(1*l + 36*s + 'Block '+ str(block.index + 1) + 35*s + 1*l)
            else:
                print(1*l + 35*s + 'Block '+ str(block.index + 1) + 35*s + 1*l)
            print(1*l + 78*'=' +1*l)
            if block.term == -1:
                print(1*l + 34*s + 'Term = '+ str(block.term) + 35*s + 1*l)
            else:
            ...
            for txn in block.txns:
                ...
                t = ' '.join(txn)
                if int(txn[2]) < 10: 
                    print(1*l + 36*s + t + 37*s + 1*l)
                else:
                    print(1*l + 36*s + t + 36*s + 1*l)
            print(1*l + 78*d + 1*l)
...
def main():
    print('Initialize server ' + sys.argv[1])
